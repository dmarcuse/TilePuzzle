// This is a bundle of compiled TypeScript code
(function () {
'use strict';
...
class Puzzle {
    ...
    validMoves() {
        ...
        if (p.y > 0)
            valid.push(Moves.UP);
        if (p.x > 0)
            valid.push(Moves.LEFT);
        if (p.y < this.size - 1)
            valid.push(Moves.DOWN);
        if (p.x < this.size - 1)
            valid.push(Moves.RIGHT);
        return valid;
    }
    /**
    ...
    shuffle(moves) {
        ...
        for (let i = 0; i < moves; i++) {
            let m = lodash.without(this.validMoves(), last);
    ...
    isSolved() {
        for (let i = 0; i < this.tiles.length; i++) {
            if (this.tiles[i] != (i + 1) % (this.sizeSq))
                return false;
        }
        return true;
    }
...
class HTMLPuzzle extends Puzzle {
    ...
    cellAt(p) {
        this.checkBounds(p);
        return this.root.querySelectorAll(".puzzle-row")[p.y].querySelectorAll(".puzzle-cell")[p.x];
    }
    /**
    ...
    applyMoves(moves) {
        ...
        this.move(moves[0]);
        if (moves.length > 1) {
            window.setTimeout(() => this.applyMoves(lodash.tail(moves)), 500);
        }
        else {
            this.inputBlocked = false;
        }
 ...
 */
class RomanPuzzle extends HTMLPuzzle {
    generateCellText(t) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.body.addEventListener("keydown", e => {
        switch (e.key) {
            case "ArrowDown":
                if (p.canMove(Moves.UP))
                    p.userMove(Moves.UP);
                break;
            case "ArrowLeft":
                if (p.canMove(Moves.RIGHT))
                    p.userMove(Moves.RIGHT);
                break;
            case "ArrowUp":
                if (p.canMove(Moves.DOWN))
                    p.userMove(Moves.DOWN);
                break;
            case "ArrowRight":
                if (p.canMove(Moves.LEFT))
                    p.userMove(Moves.LEFT);
                break;
        }
    });
